...
t = 2
n_parties = 3
party_indices = list(range(1, n_parties + 1)) # ID của các node
# ----------------------------------------------------
...
# Trusted Dealer chia sẻ key (giao thức Feldman VSS)
...
R = sum([r * G for r in nonces], INFINITY)  # Point(curve.curve, Q.x(), Q.y()) # khởi tạo bằng point Q = INFINITY vô cực --> r is k
r = R.x() % n
...
partials = []
for i, (k_i, s_i) in enumerate(zip(nonces, shares), 1):
    if i in [1,2]:
        lamda_i = lagrange_coefficient(i, [1,2])
        s_part = (numbertheory.inverse_mod(k_i, n) * (h + r * lamda_i * s_i)) % n
        partials.append(s_part)
print("Partial lagrange_coefficient signatures:", partials)
...
signature = (r, z)
print("Threshold ECDSA signature:", signature)

# --------------------------
# lagrange_coefficient verify
def verify(Q, signature, h):
    r, s = signature
    w = numbertheory.inverse_mod(s, n)
    u1 = (h*w) % n
    u2 = (r*w) % n
    P = u1*G + u2*Q
    return P != INFINITY and P.x() % n == r

print("Verify signature:", verify(Q, signature, h))
...
file_hash = msg_hash
try:
    valid = VerifyKey(pubkey).verify(file_hash, signatureECDSA)
    print("Valid signature:", valid)
except BadSignatureError:
    print("Invalid signature")


def schnorr_verify_from_public_parts(Q_point, R_point, z_scalar, msg_hash_bytes):
    """
    Verify Schnorr-like signature on SECP256k1 using group equation:
        z * G == R + h * Q
    where h = int(msg_hash_bytes) mod n (this follows the code above).
    Inputs:
      - Q_point: ecdsa.ellipticcurve.Point (public key)
      - R_point: ecdsa.ellipticcurve.Point (joint nonce)
      - z_scalar: int
      - msg_hash_bytes: bytes (the same msg_hash as used in signing)
    Returns: True/False
    """
    h_val = int.from_bytes(msg_hash_bytes, "big") % n
    left = z_scalar * G
    right = R_point + h_val * Q_point
    return left == right
